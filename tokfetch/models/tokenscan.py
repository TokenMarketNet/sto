...
from sqlalchemy.orm import Query, object_session, session
from sqlalchemy.orm.attributes import flag_modified

from tokfetch.models.utils import TimeStampedBaseModel, UTCDateTime, now
...
class _TokenHolderDelta(TimeStampedBaseModel):
    ...
    #: Raw uint256 data
    raw_delta = sa.Column(sa.LargeBinary(32), nullable=False)
    ...
    def __str__(self):
        return "<Token:{}, address:{}, block:{}, tx:{} delta:{}>".format(self.account.token.address, self.account.address, self.block_num, self.txid, self.get_delta_uint())
    ...
    def get_all_deltas(cls, status: _TokenScanStatus) -> Query:
        ...
        return object_session(status).query(cls).filter(cls.account_id.in_([ex.id for ex in status.accounts]))
    ...
    def delete_potentially_forked_block_data(cls, status: _TokenScanStatus, after_block: int):
        ...
        for acc in status.accounts:
            deltas = acc.deltas.filter(cls.block_num >= after_block).all()
            [acc.deltas.remove(delta) for delta in deltas]
            acc.mark_dirty()
...
class _TokenHolderAccount(TimeStampedBaseModel):
    ...
    raw_balance = sa.Column(sa.LargeBinary(32), nullable=True)
