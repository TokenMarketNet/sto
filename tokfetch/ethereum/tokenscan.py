...
from tokfetch.ethereum.scanner import TokenScanner
from tokfetch.ethereum.utils import get_abi, create_web3

from tokfetch.models.implementation import TokenScanStatus, TokenHolderDelta, TokenHolderAccount
...
def token_scan(logger: Logger,
    ...
    scanner = TokenScanner(logger, network, dbsession, web3, abi, token_address, TokenScanStatus, TokenHolderDelta,
                           TokenHolderAccount)
    ...
    logger.info("Current last block for chain %s: %s%s%s", network, colorama.Fore.LIGHTGREEN_EX, last_block,
                colorama.Fore.RESET)
    logger.info("Scanning blocks: %s%d%s - %s%d%s", colorama.Fore.LIGHTGREEN_EX, start_block, colorama.Fore.RESET,
                colorama.Fore.LIGHTGREEN_EX, end_block, colorama.Fore.RESET)

    if last_scanned_block:
        logger.info("Last scan ended at block: %s%d%s", colorama.Fore.LIGHTGREEN_EX, last_scanned_block,
                    colorama.Fore.RESET)
