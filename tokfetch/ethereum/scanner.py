...
from tokfetch.ethereum.utils import getLogs
from tokfetch.models.tokenscan import _TokenHolderAccount, _TokenScanStatus
...
class TokenScanner:
    ...
    NULL_ADDRESS = "0x0000000000000000000000000000000000000000"  # Used as an address in new issuances 
                                                                 # when "from" is not available 

    def __init__(self, logger: Logger, network: str, dbsession: Session, web3: Web3, abi: dict, token_address: str,
                 TokenScanStatus: type, TokenHolderDelta: type, TokenHolderAccount: type):
        ...
        self.TokenScanStatus = TokenScanStatus  #: type tokfetch.models.implementation.TokenScanStatus
        self.TokenHolderDelta = TokenHolderDelta  #: type tokfetch.models.implementation.TokenHolderDelta
        self.TokenHolderAccount = TokenHolderAccount  #: type tokfetch.models.implementation.TokenHolderAccount]
    ...
    def get_or_create_status(self) -> _TokenScanStatus:
        ...
        # "kovan", "ethereum", "testing", "ropsten")  # TODO: Sanity check - might want to remove this

        account = self.dbsession.query(self.TokenScanStatus).filter_by(network=self.network,
                                                                       address=self.address).one_or_none()
        if not account:
    ...
    def get_token_contract_info(self, token_address) -> [str, str, int, int]:
        ...
        return [contract.functions.name().call(), contract.functions.symbol().call(),
                contract.functions.decimals().call(), contract.functions.totalSupply().call()]
    ...
    def create_deltas(self, block_num: int, block_when: datetime, txid: str, idx: int, from_: str, to_: str,
                      value: int):
    ...
    def scan_chunk(self, start_block, end_block) -> Set[str]:
        ...
        # Discriminate between ERC-20 transfer
        ...
        events = getLogs(Transfer, fromBlock=start_block, toBlock=end_block)

        # AttributeDict({'args': AttributeDict({'from': '0xDE5bC059aA433D72F25846bdFfe96434b406FA85',
        # 'to': '0x0bdcc26C4B8077374ba9DB82164B77d6885b92a6', 'value': 300000000000000000000}),
        # 'event': 'Transfer', 'logIndex': 0, 'transactionIndex': 0, 'transactionHash': HexBytes(
        # '0x973eb270e311c23dd6173a9092c9ad4ee8f3fe24627b43c7ad75dc2dadfcbdf9'), 'address':
        # '0x890042E3d93aC10A426c7ac9e96ED6416B0cC616', 'blockHash': HexBytes(
        # '0x779f55173414a7c0df0d9fc0ab3fec461a66ceeee0b4058e495d98830c92abf8'), 'blockNumber': 7})
        for e in events:
            ...
            self.logger.debug("Importing %s, token:%s block:%d from:%s to:%s value:%s", e["event"], self.address,
                              e["blockNumber"], from_, e["args"]["to"], e["args"]["value"])
            self.create_deltas(e["blockNumber"], block_when, e["transactionHash"].hex(), idx, from_,
                               e["args"]["to"], e["args"]["value"])
            self.logger.debug("Imported %s, token:%s block:%d from:%s to:%s value:%s", e["event"], self.address,
                              e["blockNumber"], from_, e["args"]["to"], e["args"]["value"])
    ...
    def scan(self, start_block, end_block, start_chunk_size=20, progress_callback=Optional[Callable]) -> dict:
        ...
        while current_block <= end_block:
            ...
            self.logger.debug(
                "Scanning token transfers for blocks: %d - %d, "
                "chunk size %d, "
                "last chunk scan took %f, "
                "last logs found %d",
                current_block, current_end, chunk_size, last_scan_duration, last_logs_found)
            start = time.time()

            # Process blocks for this token over eth_getLogs
            mutated_addresses = self.scan_chunk(current_block, current_end)
            mutated_addresses.discard(self.NULL_ADDRESS)
        ...
        result = status.get_raw_balances(updated_token_holders)
        return result
